...
function formatNum(value: number | null, max = 0): string {
...
function formatValue(row: IndicatorRow | null): string {
  ...
  // Si algún día metes un valor decimal en unidad "-"
  return v.toLocaleString("es-ES", { maximumFractionDigits: 2 });
}

/**
 * Clave única de card:
 * aquí el campo "Indicador" es siempre "WTO - Sostenibilidad",
 * por lo que usamos DESCRIPCIÓN como id principal.
 */
...
const EventosSosteniblesDashboard: FC<Props> = ({
  scopeId,
  scopeName,
  data,
}) => {
  ...
  const cards = useMemo(() => {
    ...
    // Orden: primero el “Número de vías/canales...” si existe, luego “Tipo de vías/canales...”
    const arr = Array.from(map.values());
    const order = (r: IndicatorRow) => {
      const d = (r.description ?? "").toLowerCase();
      if (d.includes("número de vías") || d.includes("numero de vías"))
        return 0;
      if (d.includes("tipo de vías") || d.includes("tipo de vias")) return 1;
      return 2;
    };
      ...
      <header className="flex flex-col gap-3 border-b border-slate-200 pb-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Canales y vías de comunicación para promover turismo sostenible.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
          ...
          <div className="flex flex-col text-sm">
            ...
            <input
              ...
              placeholder="Descripción, fuente, organismo…"
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 text-xs text-slate-500 line-clamp-3">
                      {row.indicator ?? "WTO - Sostenibilidad"}
                    </div>
...
export default EventosSosteniblesDashboard;

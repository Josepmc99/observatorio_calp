...
function formatValue(row: IndicatorRow | null): string {
  ...
  // energía: kWh, MWh, GWh, tep, etc.
  // (deja 2 decimales por defecto)
  return formatNum(v, 2);
}

/**
 * Clave única:
 * - ETIS si existe y no es "-"
 * - si no, Descripción
...
export default function GestionEnergeticaDashboard({
  scopeId,
  scopeName,
  data,
}: Props) {
      ...
      <header className="flex flex-col gap-3 border-b border-slate-200 pb-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Indicadores de consumo, eficiencia y energía del destino.
          </p>
        </div>

...
const PRIMARY_SOFT = "#F97373"; // ring soft
...
function formatPct(value: number | null, max = 2): string {
  if (value == null || Number.isNaN(value)) return "—";
  return `${value.toLocaleString("es-ES", { maximumFractionDigits: max })}%`;
}

/** Formatea valor según unidad (%, Km, etc.). */
function formatValue(row: IndicatorRow | null): string {
  ...
  // % transporte (D.1.1.1 / D.1.1.2)
  if (unit.includes("%") || isEtis(row, "D.1.1.1") || isEtis(row, "D.1.1.2")) {
    return formatPct(v, 2);
  }

  // Km muy grandes: 0 decimales
  if (unit.includes("km")) {
...
/** Clave para seleccionar ficha: ETIS si existe, si no, descripción. */
...
const ReduccionImpactoTransporteDashboard: FC<Props> = ({
  scopeId,
  scopeName,
  data,
  initialYear,
  ...
  /** Primero filtramos por año + búsqueda. */
  ...
  /** En este ámbito queremos SOLO estos ETIS. */
  const transportOnly = useMemo(() => {
    ...
    return filtered.filter((r) => {
      const e = r.etis ? normEtis(r.etis) : "";
      return allowed.has(e);
    });
  }, [filtered]);

  /** “cards” deduplicadas (si el excel trae duplicados) con scoring como en Gobernanza. */
  const cards = useMemo(() => {
    ...
    for (const row of transportOnly) {
    ...
    // Orden deseado por ETIS
    const order = ["D.1.1.1", "D.1.1.2", "D.1.3", "D.1.3.1", "D.1.3.2"];
    const idx = (r: IndicatorRow) => {
      const e = r.etis ? normEtis(r.etis) : "";
      const i = order.indexOf(e);
      return i === -1 ? 999 : i;
    };

    return Array.from(map.values()).sort((a, b) => idx(a) - idx(b));
  }, [transportOnly]);
      ...
      <header className="flex flex-col gap-3 border-b border-slate-200 pb-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs text-slate-400">
            ...
            / Reducción del impacto generado por el transporte
          </p>
          <h1 className="text-2xl font-semibold text-slate-900">{scopeName}</h1>
          <p className="mt-1 text-sm text-slate-500">
            Reparto modal (privado/colectivo) y distancias recorridas.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
          ...
          <div className="flex flex-col text-sm">
            ...
            <input
              ...
              placeholder="ETIS, descripción, fuente, organismo…"
...
export default ReduccionImpactoTransporteDashboard;
